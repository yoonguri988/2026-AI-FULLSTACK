...
<%@include file="../inc/header.jsp" %>
	...
	<section class="container my-5">
		...
		<table class="table table-striped table-bordered table-hover">
			...
			<c:forEach var="board" items="${boardList}">
				<tr>
					<td>${board.rownum}</td>
					<td><a href="BDetail.do?bno=${board.bno}" title="${board.btitle}">${board.btitle}</a></td>
					<td>${board.bname}</td>
					<td>${board.bdate}</td>
					<td>${board.bhit}</td>
				</tr>
		...
		<!-- 글쓰기 버튼 -->
		<div class="text-end">
			<a href="BWriteView.do" title="글쓰기 폼" class="btn btn-primary">글쓰기</a>
		</div>
	</section>
	...
	function searchKeyup() {
		...
		fetch("./BSearch.do?search="+searchInput)
		  .then((res)=>{
		  ...
		  .then((data)=> {
			  ...
			  for(let v of data){
				  htmlData += `<tr>
				  			   <td>\${v.rownum}</td>
				  			   <td><a href=\'BDetail.do?bno=\${v.bno}\'>\${v.btitle}</a></td>
				  			   <td>\${v.bname}</td>
				  			   <td>\${v.bdate}</td>
				  			   <td>\${v.bhit}</td>
				  			   </tr>`;
