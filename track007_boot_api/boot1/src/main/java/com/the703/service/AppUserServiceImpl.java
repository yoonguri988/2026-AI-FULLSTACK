...
public class AppUserServiceImpl implements AppUserService{
	...
	@Override
	public int iddoubleByEmail(String email, String provider) { // 이메일 중복
		AppUserDto dto = new AppUserDto();
		dto.setEmail(email);
		return dao.iddoubleByEmail(dto);
	}

	// 로그인한 유저맞는지 확인 = 비번 맞는지 확인
	@Override
	public boolean matchPassword(String email, String provider, String rawPassword) {
		//1. dbUser 찾기
	...
	public int delete(AppUserDto dto, boolean local) {
		// 비번이 맞지 않는 경우, return 0 [검증]
		if(!matchPassword(dto.getEmail(), dto.getProvider(), dto.getPassword())) { return 0; }
